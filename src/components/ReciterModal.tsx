...
export const ReciterModal: React.FC<ReciterModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-lg bg-[#0F172A] border border-slate-700/80 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Mic2 className="w-5 h-5 text-emerald-400" />
            <h3 className="font-bold text-lg text-white">Choose Reciter</h3>
          </div>
          <button
            onClick={handleClose}
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between p-3.5 rounded-2xl border cursor-pointer transition-all active:scale-[0.99] ${
                  isSelected
                    ? 'bg-emerald-950/50 border-emerald-500/70 text-white shadow-md shadow-emerald-950/30 ring-1 ring-emerald-500/40'
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{reciter.description}</p>
                  </div>
                </div>
